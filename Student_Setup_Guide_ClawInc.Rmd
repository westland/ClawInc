...
3. BotFather will ask for a name and a username. Create **5 bots** one at a time:
...
Do the same for Coder, Scout, Writer, and Watcher bots.
...
## Voice Commands — Giving Verbal Orders

You can speak your commands instead of typing them, but voice transcription requires **Telegram Premium**.

**With Telegram Premium:** Hold the **microphone icon**, speak your command, release to send. The voice note is automatically transcribed and passed to the agent as text.

**Without Telegram Premium (free accounts):** Type your commands as text messages. This works identically — agents respond to text just as well as voice.

**Best practice — use Henry for multi-agent commands:**

Henry is your Chief of Staff and can delegate to the whole team from a single message. For example:
...
Henry decomposes the request, delegates to Scout and Writer in sequence, and coordinates the result.

> **Important:** Voice *calls* do not reach the agents — only voice *notes* (hold the mic button) do. And voice notes only work with Telegram Premium.
